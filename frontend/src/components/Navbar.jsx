      ...
      <div className="flex gap-2">
        ...
        <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className="flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium">
            {
              !token && !userData && <NavLink onClick={() => setShowMenu(false)} to='/login'><p className='px-4 py-2 rounded inline-block'>CREATE ACCOUNT</p></NavLink>
            }
            ...
            <button
              onClick={() => (window.location.href = "https://online-doctor-consult-admin-panel.onrender.com/")}
              className="px-6 py-2 rounded-full font-semibold text-xl"
            >
              Admin Login
            </button>
          </ul>
